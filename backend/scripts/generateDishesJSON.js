...
// All 550+ dishes from the seedManyDishes.js file
let dishes = [
...
// Synthetic dish generator to reach target count while keeping cuisine-appropriate names
const cuisineBlueprints = {
  'North Indian': {
    forms: ['Curry', 'Masala', 'Korma', 'Kebab', 'Biryani', 'Bhuna', 'Do Pyaza', 'Tikka'],
    proteins: ['Chicken', 'Mutton', 'Paneer', 'Chickpea', 'Fish', 'Vegetable', 'Eggplant'],
    adjectives: ['Spicy', 'Creamy', 'Smoky', 'Tandoori', 'Dhaba-Style', 'Home-Style']
  },
  'South Indian': {
    forms: ['Curry', 'Sambar', 'Rasam', 'Poriyal', 'Fry', 'Biryani', 'Kootu'],
    proteins: ['Chicken', 'Fish', 'Prawn', 'Vegetable', 'Egg', 'Tofu'],
    adjectives: ['Chettinad', 'Andhra-Style', 'Kerala-Style', 'Coconut', 'Spicy']
  },
  'Chinese': {
    forms: ['Stir-Fry', 'Fried Rice', 'Noodles', 'Dumplings', 'Soup', 'Hotpot'],
    proteins: ['Chicken', 'Beef', 'Pork', 'Shrimp', 'Tofu', 'Eggplant'],
    adjectives: ['Sichuan', 'Cantonese', 'Hunan', 'Garlic', 'Ginger', 'Spicy']
  },
  'Italian': {
    forms: ['Pasta', 'Risotto', 'Pizza', 'Salad', 'Panini', 'Gnocchi'],
    proteins: ['Chicken', 'Beef', 'Pancetta', 'Mushroom', 'Seafood', 'Vegetable'],
    adjectives: ['Classic', 'Truffle', 'Herbed', 'Four Cheese', 'Tomato-Basil', 'Rustic']
  },
  'Thai': {
    forms: ['Curry', 'Stir-Fry', 'Soup', 'Salad', 'Fried Rice', 'Noodles'],
    proteins: ['Chicken', 'Beef', 'Pork', 'Shrimp', 'Tofu', 'Vegetable'],
    adjectives: ['Green', 'Red', 'Massaman', 'Spicy', 'Basil', 'Lemongrass']
  },
  'Korean': {
    forms: ['BBQ', 'Stew', 'Soup', 'Rice Bowl', 'Stir-Fry', 'Pancake'],
    proteins: ['Beef', 'Pork', 'Chicken', 'Tofu', 'Seafood', 'Vegetable'],
    adjectives: ['Gochujang', 'Soy-Garlic', 'Kimchi', 'Sesame', 'Spicy']
  },
  'French': {
    forms: ['Stew', 'Gratin', 'Quiche', 'Salad', 'Roast', 'Confit'],
    proteins: ['Chicken', 'Beef', 'Duck', 'Mushroom', 'Seafood', 'Vegetable'],
    adjectives: ['Herbes de Provence', 'Creamy', 'Classic', 'Country-Style', 'Dijon']
  },
  'Continental': {
    forms: ['Grill', 'Roast', 'Stew', 'Salad', 'Sandwich', 'Skillet'],
    proteins: ['Chicken', 'Beef', 'Pork', 'Salmon', 'Mushroom', 'Vegetable'],
    adjectives: ['Herbed', 'Garlic-Butter', 'Smoky', 'Citrus', 'Peppercorn']
  }
};

const vegProteins = new Set(['Paneer', 'Tofu', 'Mushroom', 'Vegetable', 'Chickpea', 'Eggplant', 'Salad']);

const baseIngredientsByCuisine = {
  'North Indian': ['Onion', 'Tomato', 'Ginger', 'Garlic', 'Garam Masala', 'Cumin', 'Coriander'],
  'South Indian': ['Mustard Seeds', 'Curry Leaves', 'Coconut', 'Chili', 'Tamarind'],
  'Chinese': ['Soy Sauce', 'Ginger', 'Garlic', 'Scallion', 'Sesame Oil'],
  'Italian': ['Olive Oil', 'Garlic', 'Tomato', 'Basil', 'Parmesan'],
  'Thai': ['Fish Sauce', 'Lime', 'Lemongrass', 'Chili', 'Thai Basil'],
  'Korean': ['Gochujang', 'Soy Sauce', 'Garlic', 'Sesame Oil', 'Scallion'],
  'French': ['Butter', 'Garlic', 'Thyme', 'White Wine', 'Cream'],
  'Continental': ['Butter', 'Garlic', 'Herbs', 'Black Pepper', 'Olive Oil']
};

const ensureMinimumDishCount = (targetCount = 550) => {
  const nameSet = new Set(dishes.map(d => d.name + '|' + d.cuisine));
  const cuisines = Object.keys(cuisineBlueprints);
  let safety = 10000;
  while (dishes.length < targetCount && safety-- > 0) {
    const cuisine = cuisines[Math.floor(Math.random() * cuisines.length)];
    const bp = cuisineBlueprints[cuisine];
    const adjective = bp.adjectives[Math.floor(Math.random() * bp.adjectives.length)];
    const protein = bp.proteins[Math.floor(Math.random() * bp.proteins.length)];
    const form = bp.forms[Math.floor(Math.random() * bp.forms.length)];
    const name = `${adjective} ${protein} ${form}`;

    const key = name + '|' + cuisine;
    if (nameSet.has(key)) continue;
    nameSet.add(key);

    const type = vegProteins.has(protein) ? 'Veg' : 'Non-Veg';
    const ingredients = [protein, ...baseIngredientsByCuisine[cuisine]].slice(0, 7);
    dishes.push({ name, type, cuisine, ingredients });
  }
};

// Expand dishes to at least 550 items
ensureMinimumDishCount(550);
...
// Write to JSON file
const filePath = path.join(dataDir, '550-dishes-complete.json');
fs.writeFileSync(filePath, JSON.stringify(dishesWithDetails, null, 2));

console.log(`✅ Generated ${dishesWithDetails.length} dishes and saved to ${filePath}`);
...
console.log(`\n🎉 Total dishes: ${dishesWithDetails.length}`);
